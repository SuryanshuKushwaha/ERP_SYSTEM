...
const Applications = () => {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const res = await fetch('https://erp-system-jdr2.onrender.com/api/applications');
  ...
  const deleteApplication = async (id) => {
    ...
    try {
      const res = await fetch(`https://erp-system-jdr2.onrender.com/api/applications/${id}`, { method: 'DELETE' });
              ...
              <div key={a._id} className="card" style={{ marginBottom: 12, padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    {a.resumePath ? <a className="btn btn-ghost" href={`https://erp-system-jdr2.onrender.com/${a.resumePath}`} target="_blank" rel="noreferrer">Resume</a> : null}
                    {a.coverLetterPath ? <a className="btn btn-ghost" href={`https://erp-system-jdr2.onrender.com/${a.coverLetterPath}`} target="_blank" rel="noreferrer">Cover Letter</a> : null}
