...
const QuotationDashboard = () => {
  ...
  const fetchQuotations = async () => {
    try {
      setLoading(true);
  const res = await fetch('https://erp-system-jdr2.onrender.com/api/quotations');
  ...
  const markResolved = async (id) => {
    try {
    await fetch(`https://erp-system-jdr2.onrender.com/api/quotations/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'Resolved' }) });
  ...
  const deleteQuotation = async (id) => {
    ...
    try {
  await fetch(`https://erp-system-jdr2.onrender.com/api/quotations/${id}`, { method: 'DELETE' });
  try { window.location.reload(); } catch(e) {}
      fetchQuotations();
    } catch (err) { console.error(err); }
  };
